...
export const siteConfig = {
  hero: {
    description: "Empowering Micro-Level Women Entrepreneurs through Financial and Digital Literacy",
  },
  footerLinks: [
    {
      title: "Project",
      links: [
        { id: 1, title: "About IITD DMS", url: "#about" },
        { id: 2, title: "Project Overview", url: "#project" },
        { id: 3, title: "Impact Dashboard", url: "#impact" },
      ],
    },
    {
      title: "Partners",
      links: [
        { id: 5, title: "EXL", url: "#partners" },
        { id: 6, title: "NRLM", url: "#partners" },
        { id: 7, title: "State SRLMs", url: "#partners" },
      ],
    },
  ],
};
...
export const Footer = () => {
  ...
  return (
    <footer id="contact" className="w-full pb-0 bg-brand-navy text-white overflow-hidden relative">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between p-10 z-20 relative">
        <div className="flex flex-col items-start justify-start gap-y-5 max-w-xs mx-0">
          <Link href="/" className="flex items-center gap-2">
            <span className="text-xl font-bold text-white tracking-widest uppercase">IIT Delhi DMS</span>
          </Link>
        ...
        <div className="pt-5 md:w-1/2">
          <div className="flex flex-col items-start justify-start md:flex-row md:items-start md:justify-end gap-16 lg:pr-10">
            {siteConfig.footerLinks.map((column, columnIndex) => (
              <ul key={columnIndex} className="flex flex-col gap-y-2">
                <li className="mb-2 text-sm font-extrabold text-white uppercase tracking-widest">
                  {column.title}
                </li>
                {column.links.map((link) => (
                  <li
                    key={link.id}
                    className="group inline-flex cursor-pointer items-center justify-start gap-1 text-[15px]/snug text-white/70 hover:text-brand-saffron transition-colors"
                  >
                    <Link href={link.url}>{link.title}</Link>
                    <div className="flex size-4 items-center justify-center border border-white/20 rounded translate-x-0 transform opacity-0 transition-all duration-300 ease-out group-hover:translate-x-1 group-hover:opacity-100 group-hover:border-brand-saffron">
      ...
      <div className="w-full h-48 md:h-64 relative mt-12 z-0 opacity-40 mix-blend-screen pointer-events-none">
        <div className="absolute inset-0 bg-gradient-to-t from-transparent to-brand-navy z-10 from-40%" />
        <div className="absolute inset-0 mx-6">
          <FlickeringGrid
            text={tablet ? "IMPACT" : "EMPOWERMENT"}
