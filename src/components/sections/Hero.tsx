...
export default function Hero() {
  return (
    <section id="home" className="relative w-full overflow-hidden pt-0 text-black bg-white">
      <div className="relative z-10 mx-auto h-full w-full max-w-full">
        {/* Mobile layout: text on top, image below */}
        <div className="flex flex-col md:hidden">
          {/* Text content first on mobile */}
          <div className="flex flex-col px-6 pt-20 pb-8 space-y-5">
            <h1 className="font-sans font-bold text-4xl tracking-tighter text-brand-navy">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex -space-x-3">
                {[1, 2, 3, 4].map((i) => (
                  <Avatar className="size-12 border-2 border-white" key={i}>
                    <AvatarImage src={`https://images.cnippet.dev/image/upload/v1770400411/a${i}.jpg`} />
                    <AvatarFallback>W{i}</AvatarFallback>
                  </Avatar>
                ))}
              </div>
              <div className="flex flex-col font-medium text-sm text-brand-navy ml-2">
                <span className="text-base font-bold text-brand-saffron">688+</span>
                <span>Women Trained</span>
              </div>
            </div>
            <Button
              render={<a href="#impact" />}
              className="group w-fit not-disabled:inset-shadow-none flex cursor-pointer items-center justify-center gap-0 rounded-full border-none bg-transparent px-0 py-5 font-normal shadow-none hover:bg-transparent"
            >
          ...
          {/* Image below text on mobile - landscape crop, full width */}
          ...
          {/* Marquee on mobile */}
          <div className="relative border-t border-gray-100 pt-4 pb-4 overflow-hidden">
            <div className="absolute left-0 z-40 h-full w-10 bg-gradient-to-r from-white" />
            <div className="absolute right-0 z-40 h-full w-10 bg-gradient-to-l from-white" />
            <Marquee className="[--duration:30s]">
              <div className="flex items-center space-x-6 whitespace-nowrap px-4 text-xs font-bold uppercase tracking-widest text-muted-foreground">
                <span>IIT Delhi</span><span className="text-brand-saffron">•</span>
                <span>EXL</span><span className="text-brand-saffron">•</span>
                <span>Delhi SULM</span><span className="text-brand-saffron">•</span>
                <span>Haryana SRLM</span><span className="text-brand-saffron">•</span>
                <span>UP SRLM</span><span className="text-brand-saffron">•</span>
                <span>Uttarakhand SRLM</span><span className="text-brand-saffron">•</span>
                <span>HP SRLM</span><span className="text-brand-saffron">•</span>
                <span>IIT Delhi</span><span className="text-brand-saffron">•</span>
                <span>EXL</span><span className="text-brand-saffron">•</span>
              </div>
            </Marquee>
        ...
        <div className="hidden md:grid md:h-screen md:grid-cols-12">
          {/* Left image panel */}
          <div className="w-full md:col-span-6 md:h-full">
            <img
              alt="Women training session"
            ...
            <div className="w-full max-w-3xl space-y-6">
              ...
              <div className="mt-auto space-y-7">
                <div className="mt-8 flex flex-wrap items-center gap-3 lg:mt-auto">
                  <div className="flex -space-x-3">
                    {[1, 2, 3, 4].map((i) => (
                      <Avatar className="size-12 border-2 border-white transition-all duration-300 hover:grayscale-0" key={i}>
                        <AvatarImage src={`https://images.cnippet.dev/image/upload/v1770400411/a${i}.jpg`} />
                        <AvatarFallback>W{i}</AvatarFallback>
                      </Avatar>
                    ))}
